...
type
  ...
  TPHPErrorEvent = procedure (Sender : TObject; AText : AnsiString;
        AType : TPHPErrorType; AFileName : AnsiString; ALineNo : integer) of object;
           ...
           PHPEngine.HandleError(php, Err_Msg, err, error_filename, error_lineno);
             ...
             begin
               case _type of
                ...
                ErrorMessageText := Format('PHP %s:  %s in %s on line %d', [error_type_str, buffer, error_filename, error_lineno]);
                php_log_err(PAnsiChar(ErrorMessageText), p); // changed PHP4DELPHI to PHP Syhunt
             end;
 end;
...
begin
  ...
  try
    ...
    gl^.request_info.query_string := PAnsiChar(Variables.GetVariables);
       ...
       begin
        ...
        if FContentType = '' then
         gl^.request_info.content_type := 'application/x-www-form-urlencoded'
            ...
            gl^.request_info.content_type := PAnsiChar(FContentType);
       end;
